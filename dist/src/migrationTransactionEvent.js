...
const migrationTransactionEvent = async () => {
    ...
    while (true) {
        try {
            const result = await repo.getTransactionEvents(cursor);
            if (!result.length)
                break;
            const bundles = [];
            for (const record of result) {
                try {
                    const assetId = devAssetInfo.find(e => e.assetAddress.toUpperCase() === record.address.toUpperCase())?.assetId ?? null;
                    const id = `${assetId}-${record.token_id}`;
                    const document = await opensearch.getRecord(id);
                    const transform = {
                        eventTime: (0, moment_1.default)(record.block_timestamp).unix(),
                        eventType: record.is_market ? 'trade' : record.from_address === '0x0000000000000000000000000000000000000000' ? 'mint' : record.to_address === '0x0000000000000000000000000000000000000000' ? 'burn' : 'transfer',
                        ...
                        tradeType: record.is_market ? record.type : null,
                        tokens: [
                            {
                                network: record.network,
                                assetAddress: record.address,
            ...
            cursor = `${result[result.length - 1].token_id}${result[result.length - 1].transaction_hash}`;
...
//# sourceMappingURL=migrationTransactionEvent.js.map